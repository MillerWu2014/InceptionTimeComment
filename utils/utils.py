...
def read_all_datasets(root_dir, archive_name) -> Dict[str, Tuple[Any]]:
    datasets_dict = {}
    ...
    if archive_name == 'TSC':
        for dataset_name in DATASET_NAMES:
            ...
            file_name = root_dir_dataset + dataset_name  # 数据文件所在的目录, 数据文件的完整path
            x_train, y_train = readucr(file_name + '_TRAIN')
            x_test, y_test = readucr(file_name + '_TEST')
            ...
            dataset_names_to_sort.append((dataset_name, len(x_train)))   # (dateset_name train_data_size)

        dataset_names_to_sort.sort(key=operator.itemgetter(1))   # 根据train_data_size

        for i in range(len(DATASET_NAMES)):
            DATASET_NAMES[i] = dataset_names_to_sort[i][0]  # 更新DATASET_NAMES
