...
def prepare_data():
    ...
    nb_classes = len(np.unique(np.concatenate((y_train, y_test), axis=0)))  # 获取训练和测试数据总共的类别数

    # make the min to zero of labels
    y_train, y_test = transform_labels(y_train, y_test)  # 将标签进行编码：(1, 3, 5) -> (0, 1, 2)

    # save orignal y because later we will use binary
    y_true = y_test.astype(np.int64)
    y_true_train = y_train.astype(np.int64)
    # transform the labels from integers to one hot vectors
    enc = sklearn.preprocessing.OneHotEncoder()  # 将y的类别标签进行One Hot编码
...
def fit_classifier():
    input_shape = x_train.shape[1:]  # 数据的样本大小
    # NNE模型或Inception模型
    classifier = create_classifier(classifier_name, input_shape, nb_classes,
                                   output_directory)

    classifier.fit(x_train, y_train, x_test, y_test, y_true)  # 模型训练
...
if sys.argv[1] == 'InceptionTime':
    ...
    archive_name = ARCHIVE_NAMES[0]  # TSC目录
    nb_iter_ = 5

    datasets_dict = read_all_datasets(root_dir, archive_name)  # dataset_name, train_test_dataset

    # 每次迭代都会将所有的dataset进行一次完整的训练
    for iter in range(nb_iter_):
        ...
        for dataset_name in utils.constants.dataset_names_for_archive[archive_name]:
            ...
            x_train, y_train, x_test, y_test, y_true, nb_classes, y_true_train, enc = prepare_data()  # 将数据进行转换并输出转换后的数据

            output_directory = tmp_output_directory + dataset_name + '/'  # fit_classifier函数的参数
            ...
            # fit_classifier的参数：classifier_name, input_shape, nb_classes, output_directory
            # 传入的参数： "inception", x_train.shape[1:], nb_classes, output_directory
            fit_classifier()   
    ...
    for dataset_name in utils.constants.dataset_names_for_archive[archive_name]:
        ...
        # fit_classifier的参数：classifier_name, input_shape, nb_classes, output_directory
        # 传入的参数： "nne", x_train.shape[1:], nb_classes, output_directory
        fit_classifier()
    ...
    for xp in xps:
        ...
        xp_arr = get_xp_val(xp)  # 获取xr对应的参数值
        ...
        for xp_val in xp_arr:
            ...
            kwargs = {xp: xp_val}  # 组装参数, 会传入到模型的参数中

            # 开始对每个参数进行max_iterations次迭代
            for iter in range(max_iterations):
                ...
                for dataset_name in utils.constants.dataset_names_for_archive[archive_name]:
                    ...
                    classifier.fit(x_train, y_train, x_test, y_test, y_true)  # 开始模型训练
